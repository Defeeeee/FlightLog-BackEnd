...
class FlightPacksController(Controller):
    ...
    async def list_packs(self, request: Request, supabase_client: Client) -> List[FlightPack]:
        """Fetch all flight packs for the current user with remaining hours."""
        user_id = str(request.state.user.id)
        response = supabase_client.table("flight_packs").select("*").eq("user_id", user_id).execute()
        
        packs = []
        for data in response.data:
            pack_id = data["id"]
            # Get aircraft IDs for this pack
            ac_resp = supabase_client.table("flight_pack_aircraft").select("aircraft_id").eq("pack_id", pack_id).execute()
            aircraft_ids = [item["aircraft_id"] for item in ac_resp.data]
            
            # Calculate remaining hours
            # We count flights that started AFTER or ON the start_date
            # and are in the list of valid aircraft for this pack
            flights_resp = supabase_client.table("flights")\
                .select("duration")\
                .eq("user_id", user_id)\
                .in_("aircraft_id", aircraft_ids)\
                .gte("takeoff", data["start_date"])\
                .execute()
            ...
            pack = FlightPack(
                **data,
                aircraft_ids=[UUID(aid) for aid in aircraft_ids],
